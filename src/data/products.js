export const products = {
  "macbook-pro-13": {
    id: "macbook-pro-13",
    title: "MacBook Pro 13",
    seller: "Apple",
    price: "1 299 €",
    category: "Informatique",
    image: "https://picsum.photos/400",
    images: [
      "https://picsum.photos/400",
      "https://picsum.photos/401",
      "https://picsum.photos/402",
    ],
    details:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit.",
    specifications: [
      "Puce M2 avec 8 cœurs CPU et 10 cœurs GPU",
      "8GB de RAM unifiée",
      "256GB de stockage SSD",
      "Écran Retina 13,3 pouces",
      "Jusqu'à 20 heures d'autonomie",
      "macOS Ventura",
    ],
  },
  "iphone-15-pro": {
    id: "iphone-15-pro",
    title: "iPhone 15 Pro",
    seller: "Apple",
    price: "1 199 €",
    category: "Informatique",
    image: "https://picsum.photos/400",
    images: [
      "https://picsum.photos/400",
      "https://picsum.photos/403",
      "https://picsum.photos/404",
    ],
    details:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos. Lorem ipsum dolor sit amet consectetur adipisicing elit.",
    specifications: [
      "Puce A17 Pro avec 6 cœurs CPU et 6 cœurs GPU",
      "128GB de stockage",
      "Écran Super Retina XDR 6,1 pouces",
      "Triple appareil photo 48MP + 12MP + 12MP",
      "Design en titane de qualité aérospatiale",
      "iOS 17",
    ],
  },
  "dell-xps-15": {
    id: "dell-xps-15",
    title: "Dell XPS 15",
    seller: "Dell",
    price: "1 599 €",
    category: "Informatique",
    image: "https://picsum.photos/400",
    images: [
      "https://picsum.photos/400",
      "https://picsum.photos/405",
      "https://picsum.photos/406",
    ],
    details:
      "Dell XPS 15 avec processeur Intel Core i7 de 12e génération, 16GB de RAM et 512GB de stockage SSD. Écran 15,6 pouces 4K Ultra HD avec technologie InfinityEdge. Parfait pour les professionnels créatifs.",
    specifications: [
      "Intel Core i7-12700H (14 cœurs)",
      "16GB DDR5 RAM",
      "512GB SSD NVMe",
      "Écran 15,6 pouces 4K Ultra HD",
      "Carte graphique Intel Iris Xe",
      "Windows 11 Pro",
    ],
  },
  "perceuse-bosch": {
    id: "perceuse-bosch",
    title: "Perceuse sans fil Bosch",
    seller: "Bosch",
    price: "89 €",
    category: "Bricolage",
    image: "https://picsum.photos/400",
    details:
      "Perceuse-visseuse sans fil Bosch Professional GSR 18V-60. Puissance et autonomie pour tous vos projets de bricolage. Livrée avec 2 batteries 18V et un chargeur rapide.",
    specifications: [
      "Moteur sans charbon 18V",
      "2 batteries 18V 2.0Ah incluses",
      "Chargeur rapide 30 minutes",
      "Mandrin sans clé 13mm",
      "2 vitesses + mode percussion",
      "Poids : 1,3kg",
    ],
  },
  "scie-circulaire-makita": {
    id: "scie-circulaire-makita",
    title: "Scie circulaire Makita",
    seller: "Makita",
    price: "149 €",
    category: "Bricolage",
    image: "https://picsum.photos/400",
    details:
      "Scie circulaire Makita 5007MG avec moteur 1200W et lame 190mm. Guidage laser intégré et système de protection contre les redémarrages. Idéale pour les coupes précises dans le bois.",
    specifications: [
      "Moteur 1200W",
      "Lame 190mm (24 dents)",
      "Profondeur de coupe max : 66mm",
      "Guidage laser intégré",
      "Protection contre les redémarrages",
      "Poids : 4,8kg",
    ],
  },
  "tournevis-electrique": {
    id: "tournevis-electrique",
    title: "Tournevis électrique",
    seller: "Black & Decker",
    price: "45 €",
    category: "Bricolage",
    image: "https://picsum.photos/400",
    details:
      "Tournevis électrique sans fil avec batterie 4.8V rechargeable. Livré avec 30 embouts différents et un étui de rangement. Parfait pour les petits travaux d'assemblage.",
    specifications: [
      "Batterie 4.8V rechargeable",
      "30 embouts inclus",
      "Étui de rangement",
      "Chargeur inclus",
      "Vitesse variable",
      "Poids : 0,8kg",
    ],
  },
  "peugeot-208": {
    id: "peugeot-208",
    title: "Peugeot 208",
    seller: "Peugeot",
    price: "15 500 €",
    category: "Véhicules",
    image: "https://picsum.photos/400",
    details:
      "Peugeot 208 de 2021 en excellent état. 45 000 km au compteur, moteur essence 1.2 PureTech 100ch. Équipements : climatisation, GPS, caméra de recul, régulateur de vitesse.",
    specifications: [
      "Année : 2021",
      "Kilométrage : 45 000 km",
      "Carburant : Essence",
      "Puissance : 100ch",
      "Boîte : Manuelle 6 rapports",
      "Couleur : Blanc nacré",
    ],
  },
  "renault-clio": {
    id: "renault-clio",
    title: "Renault Clio",
    seller: "Renault",
    price: "13 800 €",
    category: "Véhicules",
    image: "https://picsum.photos/400",
    details:
      "Renault Clio de 2020 en très bon état. 32 000 km au compteur, moteur diesel 1.5 dCi 85ch. Équipements : climatisation automatique, GPS, radar de recul, sièges chauffants.",
    specifications: [
      "Année : 2020",
      "Kilométrage : 32 000 km",
      "Carburant : Diesel",
      "Puissance : 85ch",
      "Boîte : Manuelle 5 rapports",
      "Couleur : Gris métallisé",
    ],
  },
  "citroen-c3": {
    id: "citroen-c3",
    title: "Citroën C3",
    seller: "Citroën",
    price: "18 200 €",
    category: "Véhicules",
    image: "https://picsum.photos/400",
    details:
      "Citroën C3 de 2022 en parfait état. 18 000 km au compteur, moteur essence 1.2 PureTech 110ch. Équipements : climatisation, GPS, caméra 360°, régulateur adaptatif.",
    specifications: [
      "Année : 2022",
      "Kilométrage : 18 000 km",
      "Carburant : Essence",
      "Puissance : 110ch",
      "Boîte : Manuelle 6 rapports",
      "Couleur : Bleu",
    ],
  },

  // Produits Électroménager
  "refrigerateur-samsung": {
    id: "refrigerateur-samsung",
    title: "Réfrigérateur Samsung",
    seller: "Didier",
    price: "599 €",
    category: "Électroménager",
    image: "https://picsum.photos/400",
    details:
      "Réfrigérateur Samsung avec technologie No Frost. Capacité de 350L avec congélateur intégré. Classe énergétique A++ pour des économies d'énergie. Design moderne et fonctionnel.",
    specifications: [
      "Capacité : 350L",
      "Technologie No Frost",
      "Classe énergétique A++",
      "Congélateur intégré",
      "Affichage digital",
      "Dimensions : 60x65x185cm",
    ],
  },
  "machine-laver-bosch": {
    id: "machine-laver-bosch",
    title: "Machine à laver Bosch",
    seller: "François",
    price: "449 €",
    category: "Électroménager",
    image: "https://picsum.photos/400",
    details:
      "Machine à laver Bosch avec capacité de 9kg. Classe énergétique A+++ pour une consommation optimale. 15 programmes de lavage et système anti-vibration. Parfait pour une famille.",
    specifications: [
      "Capacité : 9kg",
      "Classe énergétique A+++",
      "15 programmes de lavage",
      "Vitesse d'essorage : 1400 tr/min",
      "Système anti-vibration",
      "Dimensions : 60x60x85cm",
    ],
  },
  "four-electrique-whirlpool": {
    id: "four-electrique-whirlpool",
    title: "Four électrique Whirlpool",
    seller: "Damien",
    price: "299 €",
    category: "Électroménager",
    image: "https://picsum.photos/400",
    details:
      "Four électrique Whirlpool multifonction avec capacité de 60L. 8 modes de cuisson et nettoyage pyrolyse. Classe énergétique A pour des économies d'énergie. Idéal pour tous types de cuisson.",
    specifications: [
      "Capacité : 60L",
      "8 modes de cuisson",
      "Nettoyage pyrolyse",
      "Classe énergétique A",
      "Thermostat électronique",
      "Dimensions : 60x60x60cm",
    ],
  },

  // Produits Immobilier
  "appartement-t3-paris": {
    id: "appartement-t3-paris",
    title: "Appartement T3",
    description: "La Fôret",
    price: "450 000 €",
    category: "Immobilier",
    image: "https://picsum.photos/400",
    details:
      "Bel appartement T3 dans le 15ème arrondissement de Paris. Entièrement rénové avec des matériaux de qualité. Séjour lumineux, cuisine équipée, 2 chambres et salle de bain. Proche transports.",
    specifications: [
      "Surface : 65m²",
      "3 pièces (2 chambres)",
      "Étage : 4ème avec ascenseur",
      "Année de construction : 1930",
      "État : Entièrement rénové",
      "Charges : 180€/mois",
    ],
  },
  "maison-jardin-lyon": {
    id: "maison-jardin-lyon",
    title: "Maison avec jardin",
    description: "La Fôret",
    price: "650 000 €",
    category: "Immobilier",
    image: "https://picsum.photos/400",
    details:
      "Belle maison individuelle avec jardin de 200m² à Lyon. 4 chambres, 2 salles de bain, garage et terrasse. Quartier calme et résidentiel. Proche écoles et commerces.",
    specifications: [
      "Surface : 140m²",
      "4 chambres",
      "2 salles de bain",
      "Jardin : 200m²",
      "Garage 2 places",
      "Année : 2015",
    ],
  },
  "studio-meuble-marseille": {
    id: "studio-meuble-marseille",
    title: "Studio meublé",
    description: "Agence immobilière Lalouette",
    price: "180 000 €",
    category: "Immobilier",
    image: "https://picsum.photos/400",
    details:
      "Studio meublé de 25m² au cœur de Marseille, 1er arrondissement. Entièrement équipé et meublé. Idéal pour investissement locatif ou résidence étudiante. Proche Vieux-Port et transports.",
    specifications: [
      "Surface : 25m²",
      "1 pièce principale",
      "Salle de bain",
      "Cuisine équipée",
      "Meublé et équipé",
      "Étage : 3ème",
    ],
  },
};

// Fonction pour récupérer un produit par ID
export const getProductById = (id) => {
  return products[id] || null;
};

// Fonction pour récupérer tous les produits d'une catégorie
export const getProductsByCategory = (category) => {
  return Object.values(products).filter(
    (product) => product.category.toLowerCase() === category.toLowerCase()
  );
};

// Fonction pour récupérer tous les produits
export const getAllProducts = () => {
  return Object.values(products);
};
